  ...
  async function generateLinks(argoDomain) {
    ...
    try {
      ...
      try {
        ...
        ISP = metaInfo.trim() || 'CF-Node';
      } catch (execErr) {
        ISP = process.env.ISP_NAME || `${NAME}-Node`;
      }
    }
...
trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${argoDomain}&fp=chrome&type=ws&host=${argoDomain}&path=%2Ftrojan-argo%3Fed%3D2560#${NAME}-${ISP}
    `;
          ...
          console.log(`${FILE_PATH}/sub.txt saved successfully with ISP: ${ISP}`);
          uplodNodes();
          resolve(subTxt);
        } catch (writeErr) {
          console.error('generate sub faild:', writeErr);
          resolve('');
        }
      }, 2000);
    });
...
module.exports = async ({ req, res, log, error }) => {
  try {
    ...
    // 根路由 - 301重定向
    if (path === '/' || path === '') {
      return res.send('Hello Appwrite!\n\nApp is running', 200, corsHeaders); 
    }
    ...
    if (path === '/status') {
      ...
      const formattedJson = JSON.stringify(statusData, null, 2);
      
      return res.send(formattedJson, 200, {
        ...corsHeaders,
